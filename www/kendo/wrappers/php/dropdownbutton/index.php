...
<div class="demo-section k-content">
    <div>
        ...
        <?php
                $defaultButton = new \Kendo\UI\DropDownButton('defaultButton');
                $defaultButton->content("Paste");
                $defaultButton->icon("paste");
                $defaultButton->click("onClick");

                $keepText = new \Kendo\UI\DropDownButtonItem();
                $keepText->text("Keep Text Only")
                    ->icon("paste-as-html")
                    ->id("paste-html");

                $pasteHTML = new \Kendo\UI\DropDownButtonItem();
                $pasteHTML->text("Paste as HTML")
                    ->icon("paste-plain-text")
                    ->id("keep-text");

                $pasteMarkdown = new \Kendo\UI\DropDownButtonItem();
                $pasteMarkdown->text("Paste Markdown")
                    ->icon("paste-markdown")
                    ->id("keep-markdown");

                $setDefault = new \Kendo\UI\DropDownButtonItem();
    ...
    <div>
        <h4>Flat Button</h4>
        <?php
                $flatButton = new \Kendo\UI\DropDownButton('flatButton');
    ...
    <div>
        ...
        <?php
                $outlineButton = new \Kendo\UI\DropDownButton('outlineButton');
    ...
    <div>
        ...
        <?php
                $primaryButton = new \Kendo\UI\DropDownButton('primaryButton');
                $primaryButton->content("Paste");
                $primaryButton->icon("paste");
                $primaryButton->click("onClick");
                $primaryButton->themeColor("primary");
                ...
                $primaryButton->addItem($keepText);
                $primaryButton->addItem($pasteHTML);
                $primaryButton->addItem($pasteMarkdown);
                $primaryButton->addItem($setDefault);
